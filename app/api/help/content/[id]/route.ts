...
// Mock database - in a real app, this would be in a separate data layer
// IMPORTANT: This array must be identical to the one in app/api/help/content/route.ts
...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const topic = helpContent.find((item) => item.id === id)

  if (!topic) {
    console.error(`Help topic with ID '${id}' not found.`)
    return NextResponse.json({ error: "Help topic not found" }, { status: 404 })
  }
