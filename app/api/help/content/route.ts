...
// Mock database - expanded with interactive help content
...
export async function GET(request: NextRequest) {
  try {
    console.log("Help content API called")
    ...
    console.log("Search params:", { query, category, tags })
    ...
    if (tags && tags.length > 0) {
      filteredContent = filteredContent.filter((item) => tags.some((tag) => item.tags.includes(tag)))
    }

    // Sort by view count (most popular first)
    filteredContent.sort((a, b) => b.viewCount - a.viewCount)

    console.log("Returning filtered content:", filteredContent.length, "items")

    return NextResponse.json(filteredContent)
  } catch (error) {
    console.error("Error in help content API:", error)
